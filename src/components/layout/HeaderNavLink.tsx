...
export function HeaderNavLink({ href, label }: HeaderNavLinkProps) {
  ...
  return (
    <Link
      href={href}
      className={cn(
        "group relative px-3 py-2 text-[13px] font-medium tracking-[-0.01em] whitespace-nowrap",
        "text-[var(--text-muted)] transition-[color,background-color,transform] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]",
        "hover:text-[var(--text-primary)] hover:-translate-y-px",
        ...
        isActive && "text-[var(--text-primary)] after:w-[calc(100%-1.25rem)] after:opacity-80"
      )}
    >
      {label}
    </Link>
